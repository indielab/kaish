...
| Unquoted comma splits a word | A bare `,` is reserved (brace expansion, list literals) — `sed -n 1,3p file` parses as two words and errors, not a silent splat. | Quote a comma-bearing argument: `sed -n "1,3p" file`, `cut -f "1,3"`, `sort -k "2,2n"` |
