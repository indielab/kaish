...
**Status:** living design doc — this file in kaish `docs/` is the canonical copy (migrated
from kaish-extras 2026-08-01; the extras copy is superseded). Cross-model reviewed
(gemini-pro + gpt-sol). **The public types and `ToolCtx` API are NOT ready to merge** —
see the review synthesis below for the issues to resolve first.
**Target:** kaish kernel (post-0.13) · **Drafted:** 2026-08-01 (Opus design agent), tuned + reviewed same day
...
- **This is a kernel feature, and the design lives here.** The doc drafted in
  kaish-extras (an accident of where the git-plugin effort started) migrates to
  kaish `docs/` as a living document; extras keeps only its git-specific consumer
  docs.
- **The latch goes away outright.** *"It never felt complete, which is why we're
  here."* No backwards compatibility: no compat shims, no `LatchRequest`
  projection, take the §E.2 rename table in one clean break. This resolves the
  §E.2 question in favor of its existing recommendation, and simplifies review
  revision 6 (migration sequencing): the compat-adapter concern was about
  *transition safety* within the migration, not long-term compat — the redraft
  should sequence the migration for reviewability, not for old readers.
- **kaijutsu is the motivating embedder.** The wiring target is kaijutsu's
  multi-session surface (model session posts requests; a human UI session or a
  clearance-model session holds approval authority), with kaish-git's write
  profile as the first in-kernel consumer. See §H for the separation-of-duties
  design this implies.
...
1. **The type system (in-process, free).** `Requester` has no method that
   produces a `Grant` (compile-fail tested, PR 4); `with_approval_authority`
